...
class MainViewModel(private val networkService: NetworkService) {
    ...
    fun getStudentClassWise(context: Context): List<Student> {
        ...
        if (isInternetConnected) {
            ...
            val listApiStudent = networkService.getStudentClassWise()

           /* for (apiStudent1 in listApiStudent) {

                val student = StudentMapper.map(apiStudent1)
